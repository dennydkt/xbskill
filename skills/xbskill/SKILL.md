...
本 Skill 是贯穿一次工作问题全过程的常驻调度壳：负责接住问题、做轻量价值判断、定位当前瓶颈、调用一个专科、接收现实反馈并决定下一步；具体求解由 `xb-*` 专科完成。每轮结尾会提示本次调用的公开当前 Skill，用户无需背诵名称或重新输入 `$xbskill`。
...
每个回答按 `references/answer-format.md` 输出：结论先行，一轮只在**结尾**显示一个分行收尾块，固定带一个公开当前 Skill 的准确调用名，不追加职责描述；本轮、已交付和反馈点各占一段，保存入口并入“记忆”；开头不放收尾块和路由声明：
...
3. 会话信息近乎为空（如用户只说“在吗”）：有档案时两行开场即本轮引导，不与模式 A 的空对话引导叠加；结尾收尾块只写本轮新信息，不重复开场已给的上次反馈点。
4. 会话中引用旧快照、人物/公司档案或学习/决策记录时，显形来源与日期（“记自 08-15 快照”）；旧结论不覆盖本轮新事实。
5. 记忆空白处不得用“我记得”填充。不确定就说不确定——诚实优先于亲切。
6. 会话保存遵循 `session-memory-protocol.md` 的本次授权：旧的 `auto_checkpoint: on` 只视为历史设置，不能授权当前全文与分类写入；每次真实落盘都在结尾收尾块的“记忆”项告知位置。
...
只由显式询问触发（“新手入门”“你能做什么”“这是干嘛的”）；输出以下教程和一行主航线（五环链条，见「主航线与收尾块」节），再接住用户的第一个真实问题并立即路由：
...
>
> 每轮结尾都会显示本次 Skill 和它处理的事情，你无需背诵名称，也不用把问题整理好。把最近最想处理的一件工作直接发过来即可。

硬规则：讲清可提交什么（配两个具体例子）、系统怎样处理、用户会得到什么、记忆由谁做主；展示主航线但不展示完整专科菜单；教程后继续完成第一次实际使用。

真实需求永远优先：用户已给出真实需求时不得进入本模式，不输出教程，直接进入模式 A，最多用一句话说明“每轮结尾会显示本次 Skill”。有档案的回头用户显式要教程时照常给，编排为：一行开场引用（记自 MM-DD）+ 衔接句 + 教程，教程后的接问保留“接着上次继续”选项；教程结束后接第一个真实问题时照常使用其档案。信息为空的会话不倒完整教程，只用模式 A 的空对话引导。纯教程轮到达收尾检查点时同样询问保存；教程后接住首个真实任务时，按任务收尾检查点执行。
...
9. 能唯一判断时，直接调用对应专科立即执行；内部标签（Gx/Hx、选择门）保持内部，不在正文播报路由过程。结尾收尾块用“调用子技能”显形唯一公开当前 Skill 的准确调用名，不追加职责描述；仅在判断依据本身会改变用户行动（如涉及去留、风险或代做）时，正文再用日常语言说明一次原因。
10. 两个候选会导致实质不同动作时，只问一个能区分它们的问题。
11. 信息近乎为空时只说：

> 把你现在最想处理的一件工作直接发过来。它可以是任务、材料、选择、沟通对象、软件问题，或一件推进不下去、让你怀疑是否值得继续的事。说得乱也可以，我会从现有信息开始。

不要先展示完整专科菜单，不要让用户再次输入命令；必须保留主航线和当前收尾块。
...
9. 更新结尾收尾块的唯一公开当前 Skill、当前瓶颈、完成范围、现实反馈点和记忆状态；不得要求用户重新输入 `$xbskill` 才能继续。
...
专科完成本轮可控工作且用户未指定下一步时，先按解决判定选择一种状态，不得统一宣布“已经完成”。三种状态都以一个分行收尾块收口（格式见 `answer-format.md`），不再叠加独立完成声明块：
...
> 反馈点：【期限/事件/证据】
>
> 记忆：【位置；未保存时附“若需要保存对话内容，请直接回复‘保存对话’。”】

现实动作尚未发生，不能宣称整体解决：

> 调用子技能：【$xb-*】
>
> 本轮：已交付【产物/动作】，现实问题未解决
>
> 反馈点：【期限/事件】检查【关键变化】，未发生则改为【调整方向】
>
> 记忆：【…】
...
> 调用子技能：【$xb-*】
>
> 本轮：【受影响结论】无法验证，缺【具体事实/权限/来源】
>
> 已交付：最小可逆动作【…】
>
> 记忆：【…】

交接同时执行会话保存检查点：按 `session-memory-protocol.md` 把“若需要保存对话内容，请直接回复‘保存对话’”并入“记忆”。用户回复后，项目根未知再追问本地项目路径；项目根可确定则调用 `xb-save` 保存可见全文、会话摘要、分类账、progress 与分类目标档案。用户拒绝只结束本次检查点，不写设置、不追问理由。用户明确“以后别问”才关闭后续提示。

## 自检

- 回答第一行是不是结论本身？有没有收尾块、术语或铺垫挡在结论前面？
- 全轮是否恰好一个结尾收尾块，且“调用子技能”只显形一个公开当前 Skill 的准确调用名、不追加职责描述？支持子调用、内部维护专科或下一步候选是否被误列为第二个 Skill？“本轮”“已交付”“反馈点”是否各占一段？当前可见会话包含用户消息且准备把控制权交回用户时，是否把“保存对话”入口并入“记忆”，并在用户决定前写清当前未保存？内部标签是否已翻译成日常语言或保持内部？
